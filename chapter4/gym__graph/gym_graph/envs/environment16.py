...
class Env16(gym.Env):
    ...
    def __init__(self):
        ...
        # Nx Graph where the nodes have features. Betweenness is allways normalized.绘制节点具有特征的位置。中间状态完全归一化。
        # The other features are "raw" and are being normalized before prediction.其他特征是“raw”的，并且在预测之前被归一化。
        self.first = None
        ...
        self.TM = None # Traffic matrix where self.TM[src][dst] indicates how many packets are sent from src to dst
                       # TM[src][dst] 指示从 src 发送到 dst 的数据包数量 source->destination
        self.sumTM = None
    ...
    def  add_features_to_edges(self):
        ...
        for node in self.graph:
            for adj in self.graph[node]:
                ...
                if not 'latency' in self.graph[node][adj][0]:
                    self.graph[node][adj][0]['latency'] = 0
                if not 'bandwidth' in self.graph[node][adj][0]:
                    self.graph[node][adj][0]['bandwidth'] = 0
                if not 'pk' in self.graph[node][adj][0]:
                    self.graph[node][adj][0]['pk'] = 0
                if not 'situated' in self.graph[node][adj][0]:
                    self.graph[node][adj][0]['situated'] = 0
    ...
    def _first_second(self):
        # Link (1, 2) recibe trafico de los links que inyectan en el nodo 1 链接（1,2）接收来自链接注入节点1的流量。
        # un link que apunta a un nodo envía mensajes a todos los links que salen de ese nodo 指向一个节点的链接会向离开该节点的所有链接发送消息
        first = list()
        second = list()
    ...
    def allocate_to_destination_sp(self, src, dst, init_source, final_destination): 
        ...
        while (j < len(currentPath)):
            ...
            print(self.graph[firstNode][secondNode][0])
            #print("edge_state is:",self.edge_state)
    ...
    def mark_action_sp(self, src, dst, init_source, final_destination): 
        ...
        # 在此函数中，我们在 src，dst 之间的 SP 的相应边缘中设置动作
        bw_allocate = self.TM[init_source][final_destination]
        currentPath = self.shortest_paths[src, dst]
