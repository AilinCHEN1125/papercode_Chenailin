...
import sys
sys.path.append("..")
from gym__graph.gym_graph import envs
import random
...
percentage_demands = 15  # 将在优化中使用的主要需求百分比,在文章中是15%
str_perctg_demands = str(percentage_demands)
percentage_demands /= 100

EVALUATION_EPISODES = 20  # 由于需求选择是确定性的，因此在同一流量矩阵上多次评估是没有意义的
PPO_EPOCHS = 8
...
BUFF_SIZE = num_samples_top1 + num_samples_top2 + num_samples_top3  # 经验池大小 Careful to don't have more samples from one TM!

# The DECAY_STEPS must be a multiple of args.e (episode_iters)
DECAY_STEPS = 60  # 60个epsilon后学习率衰减
DECAY_RATE = 0.96
...
hidden_init_actor = tf.keras.initializers.Orthogonal(gain=np.sqrt(2), seed=SEED)    # 生成正交矩阵的初始化器，这个动作是离散的？
...
def old_cummax(alist, extractor):
    with tf.name_scope('cummax'):
        maxes = [tf.reduce_max(extractor(v)) + 1 for v in alist]    # tf.reduce_max()，取一行（或列）的最值
...
def decayed_learning_rate(step):
    lr = hparams['learning_rate'] * (DECAY_RATE ** (step / DECAY_STEPS))   # 自定义的学习率衰减公式
    if lr < 10e-5:
        lr = 10e-5    # 学习率最小值
    return lr
...
class PPOActorCritic:
    def __init__(self):
        self.memory = deque(maxlen=BUFF_SIZE)   # 经验回放,deque?
        self.inds = np.arange(BUFF_SIZE)
        self.listQValues = None                 # Q表
        self.softMaxQValues = None              # softmax Q值
        ...
        self.optimizer = tf.keras.optimizers.Adam(learning_rate=hparams['learning_rate'], beta_1=0.9, epsilon=1e-05)  # 优化器
        self.actor = actor.myModel(hparams, hidden_init_actor, kernel_init_actor)   # 后两个参数随机创建
        self.actor.build()

        self.critic = critic.myModel(hparams, hidden_init_critic, kernel_init_critic)
        self.critic.build()

    # 用于预测动作空间分布，指定环境/源节点/目的节点
    def pred_action_distrib_sp(self, env, source, destination):
        # List of graph features that are used in the cummax() call 调用cummax()中使用的图形功能列表
        list_k_features = list()

        # We get the K-middlepoints between source-destination 得到源-目的之间的K个中间节点
        middlePointList = env.src_dst_k_middlepoints[str(source) + ':' + str(destination)]
        itMidd = 0
        print(middlePointList)

        # 2. Allocate (S,D, linkDemand) demand using the K shortest paths
        # 使用 K 条最短路径分配（S，D, linkDemand）需求
    ...
    def get_graph_features(self, env, source, destination):
        ...
        sample = {
            ...
            'utilization': tf.convert_to_tensor(value=np.divide(self.utilization_feature, env.edge_state[:, 1]),
                                                dtype=tf.float32),
            'first': env.first,
            'second': env.second
        }    # 应该是字典
    ...
    def critic_get_graph_features(self, env):
        """
        迭代转换后的图节点并获取特征。容量和带宽分配的功能动态归一化
        """
        self.utilization_feature = env.edge_state[:, 0]
    ...
    def _write_tf_summary(self, actor_loss, critic_loss, final_entropy):
        with summary_writer.as_default():
            ...
            tf.summary.histogram(name='CRITIC/FirstLayer/kernel:0', data=self.critic.variables[0],
    ...
    def _critic_step(self, ret, link_state_critic, first_critic, second_critic, num_edges_critic):
        ret = tf.stop_gradient(ret)     # 对反向梯度进行截断
    ...
    def _actor_step(self, advantage, old_act, old_policy_probs, link_state, graph_id, \
        ...
        return loss_sample, entropy_sample     # 输出a_loss
    ...
    def _train_step_combined(self, inds):
        ...
        with tf.GradientTape() as tape:
            for minibatch_ind in inds:
                sample = self.memory[minibatch_ind]   # 从经验回放池中取出一个minibatch的数据进行优化

                # ACTOR
                loss_sample, entropy_sample = self._actor_step(sample["advantage"], sample["old_act"],
                                                               sample["old_policy_probs"], \
                                                               sample["link_state"], sample["graph_id"],
                                                               sample["first"], sample["second"], sample["num_edges"])
                actor_losses.append(loss_sample)
                entropies.append(entropy_sample)

                # CRITIC
                critic_sample_loss = self._critic_step(sample["return"], sample["link_state_critic"],
                                                       sample["first_critic"], sample["second_critic"],
                                                       sample["num_edges_critic"])
...
def get_advantages(values, masks, rewards):
    returns = []
    gae = 0
    print("---------------", len(values), len(masks), len(rewards))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-i', help='iters', type=int, required=True, default=50)
    parser.add_argument('-c', help='counter model', type=int, required=True)
    parser.add_argument('-e', help='episode iterations', type=int, required=True, default=50)
    ...
    env_training1.generate_environment(dataset_folder_name1 + "/TRAIN", "BtAsiaPac", EPISODE_LENGTH, NUM_ACTIONS,
                                       percentage_demands)
    env_training1.top_K_critical_demands = take_critic_demands

    env_training2 = gym.make(ENV_NAME)
    env_training2.seed(SEED)
    env_training2.generate_environment(dataset_folder_name2 + "/TRAIN", "Garr199905", EPISODE_LENGTH, NUM_ACTIONS,
                                       percentage_demands)
    env_training2.top_K_critical_demands = take_critic_demands

    env_training3 = gym.make(ENV_NAME)
    env_training3.seed(SEED)
    env_training3.generate_environment(dataset_folder_name3 + "/TRAIN", "Goodnet", EPISODE_LENGTH, NUM_ACTIONS,
                                       percentage_demands)
    env_training3.top_K_critical_demands = take_critic_demands

    env_eval = gym.make(ENV_NAME)
    env_eval.seed(SEED)
    env_eval.generate_environment(dataset_folder_name1 + "/EVALUATE", "BtAsiaPac", EPISODE_LENGTH, NUM_ACTIONS,
                                  percentage_demands)
    env_eval.top_K_critical_demands = take_critic_demands

    env_eval2 = gym.make(ENV_NAME)
    env_eval2.seed(SEED)
    env_eval2.generate_environment(dataset_folder_name2 + "/EVALUATE", "Garr199905", EPISODE_LENGTH, NUM_ACTIONS,
                                   percentage_demands)
    env_eval2.top_K_critical_demands = take_critic_demands

    env_eval3 = gym.make(ENV_NAME)
    env_eval3.seed(SEED)
    env_eval3.generate_environment(dataset_folder_name3 + "/EVALUATE", "Goodnet", EPISODE_LENGTH, NUM_ACTIONS,
                                   percentage_demands)
    ...
    # 从先前训练好的模型中加载最大奖励和最新学习率，即从最后训练好的模型开始接着训练
    if os.path.exists("./tmp/" + differentiation_str + "tmp.pckl"):
        f = open("./tmp/" + differentiation_str + "tmp.pckl", 'rb')
        max_reward, hparams['learning_rate'] = pickle.load(f)       # 得到最大奖励与学习率
        f.close()
    else:
        max_reward = -1000
    ...
    for iters in range(args.e):   # 迭代
        ...
        while not number_samples_reached:
            ...
            while 1:
                ...
                # 预测中间节点的概率，pred-预测。 参数：环境，源节点，目的节点
                ...
                reward, done, _, new_demand, new_source, new_destination, _, _, _ = env_training1.step(action, demand,
                                                                                                       source,
                                                                                                       destination)
                mask = not done
        ...
        while not number_samples_reached:
            ...
            while 1:
                ...
                # Allocate the traffic of the demand to the paths to middlepoint
                reward, done, _, new_demand, new_source, new_destination, _, _, _ = env_training2.step(action, demand,
                                                                                                       source,
                                                                                                       destination)
                mask = not done
        ...
        while not number_samples_reached:
            ...
            while 1:
                # Used to clean the TF cache 清除TF缓存
                tf.random.set_seed(1)
                # Predict probabilities over middlepoints 预测中间节点的概率
                ...
                # Allocate the traffic of the demand to the paths to middlepoint
                reward, done, _, new_demand, new_source, new_destination, _, _, _ = env_training3.step(action, demand,
                                                                                                       source,
                                                                                                       destination)
                mask = not done
                ...
                # If we have enough samples 如果有足够样本
        ...
        actor_loss, critic_loss = agent.ppo_update(actions, actions_probs, tensors, critic_features, returns,
                                                   advantages)
        ...
        for eps in range(EVALUATION_EPISODES):
            ...
            while 1:
                ...
                action = np.argmax(action_dist)
                reward, done, error_eval_links, demand, source, destination, maxLinkUti, minLinkUti, utiStd = env_eval2.step(
                    action, demand, source, destination)
                rewardAddTest += reward
                if done:
                    break
        ...
        for eps in range(EVALUATION_EPISODES):
            ...
            while 1:
                ...
                action = np.argmax(action_dist)
                reward, done, error_eval_links, demand, source, destination, maxLinkUti, minLinkUti, utiStd = env_eval3.step(
                    action, demand, source, destination)
                rewardAddTest += reward
                if done:
                    break
